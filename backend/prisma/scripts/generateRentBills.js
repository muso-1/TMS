...
async function generateMonthlyRentBills() {
  ...
  for (const lease of leases) {
    ...
    // 3️⃣ Create the bill
    const dueDate = new Date(today.getFullYear(), today.getMonth(), 5) // e.g., 5th of the month
    const bill = await prisma.rentBill.create({
      data: {
        leaseId: lease.id,
        amount: lease.monthlyRent, // 🪄 dynamic rent from lease
        dueDate
      }
    })
